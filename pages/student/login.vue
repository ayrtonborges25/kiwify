...
const registerPath = computed(() => `/student/register?clubId=${encodeURIComponent(clubId.value)}&redirect=${encodeURIComponent(redirect.value)}`)
      ...
      <div>
        ...
        <div class="mt-2 flex items-center justify-end">
          <div class="text-sm leading-5">
            <NuxtLink :to="`/forgot-password?target=student&email=${encodeURIComponent(email)}&redirect=${encodeURIComponent(redirect)}`" class="font-medium text-indigo-600 hover:text-indigo-500 focus:outline-none focus:underline transition ease-in-out duration-150">
              Esqueceu a senha?
            </NuxtLink>
          </div>
        </div>
