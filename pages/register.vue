...
const submit = async () => {
  ...
  errorMessage.value = ''
  try {
    await signUp(email.value, password.value, { name: name.value, role: 'admin' })
    await signIn(email.value, password.value)
    setLastContext('producer')
    await navigateTo(redirect.value)
  } catch (error: any) {
    errorMessage.value = error?.message || 'Nao foi possivel criar a conta.'
  } finally {
    loading.value = false
  }
}
      ...
      <div>
        <div class="sm:mx-auto sm:w-full sm:max-w-md">
          ...
          <p class="mt-2 text-center text-base leading-5 text-gray-600">
            Ou
            <NuxtLink :to="`/login?redirect=${encodeURIComponent(redirect)}`" class="font-medium text-indigo-600 hover:text-indigo-500 focus:outline-none focus:underline transition ease-in-out duration-150">
              entrar
            </NuxtLink>
          </p>
        </div>
